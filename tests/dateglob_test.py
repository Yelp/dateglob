...
class TestStrftime(TestCase):
    ...
    def test_month_globbing(self):
        ...
        self.assertEqual(
            strftime(m(2010, 6), "%b %d, %Y"), [date(2010, 6, 1).strftime("%b *, 2010")]
        )
        self.assertEqual(
            strftime(m(2010, 6), "%B %d, %Y"), [date(2010, 6, 1).strftime("%B *, 2010")]
        )
    ...
    def test_month_no_globbing(self):
        ...
        self.assertEqual(
            strftime(m(2010, 6), "%Y-%j"), [d.strftime("%Y-%j") for d in m(2010, 6)]
        )

        # can't glob day-of-week (%a, %A, %W)
        self.assertEqual(
            strftime(m(2010, 6), "%Y-%a"),
            sorted({d.strftime("%Y-%a") for d in m(2010, 6)}),
        )
        self.assertEqual(
            strftime(m(2010, 6), "%Y-%A"),
            sorted({d.strftime("%Y-%A") for d in m(2010, 6)}),
        ...
        self.assertEqual(
            strftime(m(2010, 6), "%Y-%w"),
            sorted({d.strftime("%Y-%w") for d in m(2010, 6)}),
        )

        # can't glob whole date (%c, %x)
        self.assertEqual(
            strftime(m(2010, 6), "%c"), sorted(d.strftime("%c") for d in m(2010, 6))
        )
        self.assertEqual(
            strftime(m(2010, 6), "%x"), sorted(d.strftime("%x") for d in m(2010, 6))
        )
    ...
    def test_year_globbing(self):
        ...
        self.assertEqual(strftime(y(2010), "%Y: %a, %A, %j, %w"), ["2010: *, *, *, *"])

    def test_year_no_globbing(self):
        # can't glob whole date (%c, %x)
        self.assertEqual(
            strftime(y(2010), "%c"), sorted(d.strftime("%c") for d in y(2010))
        )
        self.assertEqual(
            strftime(y(2010), "%x"), sorted(d.strftime("%x") for d in y(2010))
        )

    def test_ten_globbing(self):
        # Test first ten
        self.assertEqual(
            strftime(t(2016, 5, 1), "%Y-%m-%d"),
            ["2016-05-0*", "2016-05-10", "2016-05-11"],
        )
        # Test second ten
        self.assertEqual(
            strftime(t(2016, 5, 10), "%Y-%m-%d"), ["2016-05-1*", "2016-05-20"]
        )
        # Test third ten
        self.assertEqual(
            strftime(t(2016, 5, 20), "%Y-%m-%d"), ["2016-05-2*", "2016-05-30"]
        )

        # Test fourth ten on 30-day month
        self.assertEqual(
            strftime(t(2016, 4, 30, period=2), "%Y-%m-%d"), ["2016-04-3*", "2016-05-01"]
        )
        # Test fourth ten on 31-day month with all days
        self.assertEqual(strftime(t(2016, 1, 30, period=2), "%Y-%m-%d"), ["2016-01-3*"])
        # Test fourth ten on 31-day month with one of the 2 days
        self.assertEqual(strftime(t(2016, 1, 30, period=1), "%Y-%m-%d"), ["2016-01-30"])
        # Test fourth ten on 31-day month with one of the 2 days
        self.assertEqual(strftime(t(2016, 1, 31, period=1), "%Y-%m-%d"), ["2016-01-31"])

        # Test February of Leap Year
        self.assertEqual(
            strftime(t(2016, 2, 20, period=10), "%Y-%m-%d"), ["2016-02-2*"]
        )
        # Test February of Normal Year
        self.assertEqual(strftime(t(2015, 2, 20, period=9), "%Y-%m-%d"), ["2015-02-2*"])
    ...
    def test_sorting(self):
        ...
        dates = (
            [date(2010, 6, 6)] + list(m(2007, 7)) + list(y(2011)) + [date(2007, 5, 6)]
        )
        ...
        self.assertEqual(
            strftime(dates, "%m/%d/%y"), ["*/*/11", "05/06/07", "06/06/10", "07/*/07"]
        )
    ...
    def test_time_fields_are_globbed(self):
        self.assertEqual(
            strftime([date(2010, 6, 6)], "%Y-%m-%dT%H:%M:%SZ"), ["2010-06-06T*:*:*Z"]
        )

        self.assertEqual(strftime(y(2010), "%Y-%m-%d %f%I%p%S%X%z%Z"), ["2010-*-* *"])
